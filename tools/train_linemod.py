...
def train(net, optimizer, dataloader, epoch):
    ...
    for idx, data in enumerate(dataloader):
        ...
        vals=(float(loss_seg),float(loss_vertex),float(precision),float(recall))
...
def train_net():
    ...
    if args.test_model:
        ...
        #val_loader = DataLoader(val_set, batch_sampler=val_batch_sampler, num_workers=4)
        ...
        if args.linemod_cls in cfg.occ_linemod_cls_names:
            ...
            occ_val_loader = DataLoader(occ_val_set, batch_sampler=occ_val_batch_sampler, num_workers=4)
